...
from hardware import CrossingGate
...
class DemoCrossingGateProgram(Program):
    def __init__(self, crossing_gate: CrossingGate):
        ...
        self.pos = self.crossing_gate.get_pos()
        self.name = "차단기_" + self.pos
        ...
        topic_dispatcher = {
            f"demo/hardware/crossing_gate/{self.direction}/to/broken": self.handle_broken,
        }
    ...
    def handle_broken(self, topic, data, publisher):
        ...
        if data == '고장':
            self.demo_publisher.demo_print(f"[{self.name}] 현재 상태 : {self.crossing_gate.get_status()}")
            self.crossing_gate.set_status('고장')
            self.demo_publisher.demo_print(f"[{self.name}] {self.crossing_gate.get_status()}으로 변경되었습니다.")
        elif data == '정상':
            self.demo_publisher.demo_print(f"[{self.name}] 현재 상태 : {self.crossing_gate.get_status()}")
            self.crossing_gate.set_status('정상')
            self.demo_publisher.demo_print(f"[{self.name}] {self.crossing_gate.get_status()}으로 변경되었습니다.")
