...
def frames_hsv_to_rgb(frames):
    g_frames = []
    for frame in frames:
        g_frames.append(cv2.cvtColor(frame, cv2.COLOR_HSV2BGR))
    return np.array(g_frames, dtype='uint8')

def find_lowest_point(prev, next, x, y, w, z, radius):
    min_x, min_y = w, z
    try:
...
def three_step_search(frames, radius=4):
    ...
    for i in range(frames.shape[0]):
        ...
        if prev is None:
            ...
            for j, point in enumerate(prevPt):
                ...
                c, d = find_lowest_point(prev, next, a, b, a, b, radius)
                c, d = find_lowest_point(prev, next, a, b, c, d, int(radius/2))
                c, d = find_lowest_point(prev, next, a, b, c, d, int(radius/4))
                try:
                    # 이전 코너와 새로운 코너에 선그리기
